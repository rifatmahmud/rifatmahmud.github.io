...
THREEx.RendererStats	= function (){
	...
	msText.style.cssText = 'color:#f00;font-family:Helvetica,Arial,sans-serif;font-size:11px;font-weight:bold;line-height:15px';
	msText.innerHTML= 'WebGLRenderer';
	msDiv.appendChild( msText );
	
	var msTexts	= [];
	// var nLines	= 9; 
	var nLines	= 20; // INSIDEMAPS MODIFICATION
	...
	return {
		...
		update: function(webGLRenderer){
			// sanity check
			if (webGLRenderer instanceof THREE.WebGLRenderer) {
				...
				// BEGIN INSIDEMAPS MODIFICATIONS
				msTexts[i++].textContent = "== User =====";
				if (webGLRenderer.userData && webGLRenderer.userData.info) {
					var maxCount = 10;
					for (var key in webGLRenderer.userData.info) {
						if (maxCount-- <= 0) {
							return;
						}
						msTexts[i++].textContent = '' + key + ': ' + webGLRenderer.userData.info[key];
					}
				}
				// END INSIDEMAPS MODIFICATIONS
			}
